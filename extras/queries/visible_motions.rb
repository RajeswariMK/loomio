  ...
  def initialize(user: nil, groups: nil, group_ids: nil)
    ...
    if @user.present? && !group_ids.empty?
      @relation = @relation.where("discussions.group_id IN (:group_ids) AND
                                  (discussions.group_id IN (:user_group_ids) OR groups.privacy = 'everyone'
      ...
      @relation = @relation.where("discussions.group_id IN (:group_ids) AND groups.privacy = 'everyone'",
                                  group_ids: group_ids)
    else
      @relation = []
    end
